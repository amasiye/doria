...
- Stage 20a const-evaluable Copy-scalar defaults are complete for free functions, instance methods, static methods, and constructors through one caller-side MIR splice. Writable Copy scalars are supported; string defaults and move/`take` defaults retain explicit temporary diagnostics.
