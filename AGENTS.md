...
- Stages 11 through 16 are complete. The debug interpreter, Cranelift fast profile, and LLVM release profile consume the same validated typed MIR; the durable manifest at `crates/doriac/tests/fixtures/native_parity_examples.txt` drives exact stdout/stderr/status triple differential tests and must cover every finite `examples/native/*.doria` fixture. General control flow, recursion, fixed-width numerics, runtime bool values, explicit numeric conversions, and immutable runtime strings share that path. `doria-rt` owns native entry, exact output, canonical display conversion, refcounted string storage, and abort-only panic behavior. Stage 17 `std::io` and formatted I/O is next. Do not recreate a parallel native IR, bypass shared MIR validation, or execute user code as a native-compilation preflight.
...
- Full arbitrary-expression interpolation and `Displayable` remain Stage 18. General runtime I/O remains Stage 17, and `Bytes` remains Stage 23.
...
When native backend work changes supported behavior, run linker-independent Cranelift and LLVM object tests plus the complete durable interpreter/Cranelift/LLVM differential suite. The current Stage 16 profiles support top-level free functions, one parameterless int/void `main`, path-sensitive returns, recursion, supported structured control flow, fixed-width numerics, runtime bool values, scalar and string locals/parameters/returns/calls, string concatenation/comparison/display, bool short-circuiting, accepted explicit numeric conversions, and abort-only panic. Normal interpretation has no artificial block or call-depth cap, and native compilation has no interpreter preflight. Only `main(): int` crosses the `0..125` process-status boundary. The durable manifest must include every finite native example and compare exact stdout, stderr, and status. Do not treat the internal `doria-rt` ABI as stable or claim classes, collections, general ownership checking, or Stage 17 I/O support.
