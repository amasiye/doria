...
When a native backend target exists, native smoke tests must be part of the relevant definition of done. The current native target is the Stage 5a Cranelift-backed smoke backend for final returns, narrow terminal `if` / `else` returns, guard-style `if` returns with fallback returns, accepted boolean conditions, and direct-body writable integer local assignments only; do not treat it as full native code generation, full native mutable-variable support, or full native control-flow support.
