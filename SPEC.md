...
Binary file I/O is planned for Stage 23 as `read_file_bytes(): Bytes` and `write_file_bytes(...)`. `File` and stream objects, including RAII close and buffered/seekable access, are planned after Stage 29. These future tiers do not change the Stage 17 text and EOF contracts.
