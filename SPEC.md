...
Native execution currently supports omitted trailing defaults when the parameter is a fixed-width integer, float, or bool and the default is accepted by the Stage 20 constant-evaluation tier. This applies uniformly to free functions, instance methods, static methods, and constructors. A writable Copy-scalar parameter may use such a default because writability does not change its ownership classification. The compiler inserts the folded value at each omitted call position before MIR execution.

Const-literal string defaults are accepted language direction and a committed near-future addition, but the current compiler reports them as not yet supported. Defaults for move types and `take` parameters remain deferred until their construction and destruction obligations are implemented. Non-constant defaults are rejected before MIR. Named arguments remain separate future work.
