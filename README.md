...
- Emits Stage 2c Cranelift-backed native smoke executables for `function main(): int` using supported readonly integer locals and `+`/`-`/`*` arithmetic, with a final supported integer expression return in the accepted `0..125` portable exit-code range.
...
cargo run -p doriac -- compile examples/native/main_arithmetic_42.doria
./main_arithmetic_42
```
